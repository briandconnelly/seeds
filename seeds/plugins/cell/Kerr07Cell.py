...
class Kerr07Cell(Cell):
    ...
    death_sensitive
        death rate of SENSITIVE cells (float, [0,1])
    death_resistant
        death rate of RESISTANT cells (float, [0,1])
    death_producer
        death rate of PRODUCER cells (float, [0,1])
    toxicity
        The toxicity of PRODUCER cells (float, [0,1])
    ...
    def __init__(self, experiment, population, id, type=-1):
        ...
        *type*
        ...
        if self.ds < 0:
            raise ConfigurationError("Kerr07Cell: death_sensitive parameter must be least 0")
        elif self.ds > 1:
            raise ConfigurationError("Kerr07Cell: death_sensitive parameter must not exceed 1")
        elif self.dr < 0:
            raise ConfigurationError("Kerr07Cell: death_resistant parameter must be least 0")
        elif self.dr > 1:
            raise ConfigurationError("Kerr07Cell: death_resistant parameter must not exceed 1")
        elif self.dp < 0:
            raise ConfigurationError("Kerr07Cell: death_producer parameter must be least 0")
        elif self.dp > 1:
            raise ConfigurationError("Kerr07Cell: death_producer parameter must not exceed 1")
        elif self.tp < 0:
            raise ConfigurationError("Kerr07Cell: toxicity parameter must be least 0")
        elif self.tp > 1:
            raise ConfigurationError("Kerr07Cell: toxicity parameter must not exceed 1")

        self.type_colors = ['#777777','b','g','r']

    def __str__(self):
        """Produce a string to be used when the object is printed"""
        if self.type == self.PRODUCER:
            s = 'Kerr07 Cell %d Type %d (%s, toxicity=%f)' % (self.id, self.type, self.types[self.type], self.tp)
        else:
            s = 'Kerr07 Cell %d Type %d (%s)' % (self.id, self.type, self.types[self.type])
        return s
